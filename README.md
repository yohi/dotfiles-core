...
├── docs/                       <-- ドキュメント類（アーキテクチャ規約、エージェント用ガイド）
...
- **物理 PC（無人インストール）**: [docs/pxe-server.md](docs/pxe-server.md) に従って Docker ベースの PXE サーバーを起動し、ネットワークブートによる自動 OS インストール・初期構成を行います。
- **VPS / リモートサーバー**: [docs/ansible.md](docs/ansible.md) に従い、操作 PC から Ansible プレイブック（`ansible/setup.yml`）を実行します。

詳細な操作手順、環境変数設定、セキュリティ設計は [`docs/ansible.md`](docs/ansible.md) および [`docs/pxe-server.md`](docs/pxe-server.md) を参照してください。
