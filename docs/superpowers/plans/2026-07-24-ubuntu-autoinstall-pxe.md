...
> **Post-implementation note:** This plan is preserved as the original design record. Actual implementation deviated from some literal example code below after real-world testing surfaced bugs in the plan's own assumptions. The corrected, authoritative behavior is in the code (see inline comments) and is summarized in `.superpowers/sdd/progress.md`'s task-by-task ledger and final-review sections. Known deviations: **(1)** `fetch_netboot`'s ISO/tarball filenames are discovered dynamically (Ubuntu's real filenames include a point-release number, e.g. `ubuntu-24.04.4-...`) and echoed as `ISO=`/`TARBALL=` on stdout, not the fixed `ubuntu-${version}-...` names shown in Task 1's example. **(2)** `render_autoinstall`'s output file is named `user-data` (per cloud-init's NoCloud datasource hardcoded-filename requirement), not `autoinstall.yaml` as shown in Task 2's example — this was a Critical bug caught only in final whole-branch review. **(3)** `dnsmasq.conf.tmpl`'s `dhcp-range` line omits the interface token shown in Task 4's example (empirically confirmed both forms parse identically via `dnsmasq --test`, but the interface-less form is clearer). **(4)** `verify_sha256` matches Ubuntu's real `SHA256SUMS` binary-mode format (`<hash> *<filename>`), not only the two-space format shown in Task 1's example — also a Critical fix from final review.
