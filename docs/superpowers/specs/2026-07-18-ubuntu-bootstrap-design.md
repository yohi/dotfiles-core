...
2. `curl -fsSL https://setup.yourdomain.com/install.sh | sh` を実行する。
...
- スクリプト内で取得する GitHub 鍵は `https://github.com/yohi.keys` のみとし、取得内容に改行や不正な文字が含まれないことを検証する。
...
3. **簡易整合性チェック**
   - 取得した内容が空でないことを確認する。
   - 内容の先頭が `#!/bin/bash` または `#!/bin/sh` であることを確認する。
...
- 対話式入力されたトークンは、実行時に Ansible の `-e` オプションまたは一時的な環境変数経由で渡す。
...
- クライアント側で `X-Script-SHA256` を使った任意検証を可能にする。
...
- `github_token` は `vars.yml` への保存を避け、実行時に環境変数または対話入力で受け取る。
...
- チェックサム検証付きの配信（Workers 側での署名ではなく、クライアント側での検証）。
- スクリプト内容の GPG 署名。
