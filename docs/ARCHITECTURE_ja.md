...
各コンポーネント（`components/dotfiles-*`）のルートディレクトリに `.env` ファイルを配置することで、コンポーネント固有の環境変数を定義できる。

### 記述ルール（規約）

1. **単純な変数代入のみ**: `KEY=VALUE` 形式の単純な代入のみを許容する。
2. **シェルスクリプトの禁止**: `source` される可能性があるが、条件分岐、ループ、コマンド実行（バッククォートや `$()`）などのロジックを含めてはならない。
3. **目的**: コンポーネントの Makefile やシェル環境で共通利用する「設定値」の定義に限定する。

> [!CAUTION]
> 本ファイルは Orchestrator の Makefile および dotfiles-zsh の初期化時に自動的に読み込まれる。
> 規約に反する記述（実行コマンド等）が含まれている場合、環境の破損や予期せぬ動作を招く恐れがある。
